...
class H5PContentAdmin {
  ...
  public function alter_title($page, $admin_title, $title) {
    // Use WordPress sanitization instead of the deprecated FILTER_SANITIZE_STRING
    $task = isset($_GET['task']) ? sanitize_text_field(wp_unslash($_GET['task'])) : NULL;
  ...
  public function process_new_content($echo_on_success) {
    ...
    $action = filter_input(INPUT_POST, 'action', FILTER_VALIDATE_REGEXP, ['options' => ['regexp' => '/^(upload|create)$/']]);
    if ($action) {
      ...
      if ($action === 'create') {
        ...
        $content             = ($this->content === NULL ? ['disable' => H5PCore::DISABLE_NONE] : $this->content);
  ...
  private function set_content_tags($content_id, $tags = '') {
    ...
    if (is_array($tags)) {
      ...
      // Ensure we have a string and handle empty input without calling explode on non-strings
      $tags       = (string) $tags;
      $tags       = trim($tags);
      $tags_array = ($tags === '' or $tags === NULL) ? [] : explode(',', $tags);
    }
    ...
    foreach ($tags_array as $tag) {
      ...
      if (empty($exists)) {
        // Create tag
        $exists = ['name' => $tag];
        $wpdb->insert("{$wpdb->prefix}h5p_tags", $exists, ['%s']);
      ...
      if (empty($exists->content_id)) {
        // Connect to content
        $wpdb->insert("{$wpdb->prefix}h5p_contents_tags", ['content_id' => $content_id, 'tag_id' => $exists->id], ['%d', '%d']);
      }
    }
  ...
  public function ajax_contents($insert = FALSE) {
    ...
    if ($insert) {
      $fields = ['title', 'content_type', 'user_name', 'tags', 'updated_at', 'id', 'user_id', 'content_type_id', 'slug'];
    } else {
      $fields = ['title', 'content_type', 'user_name', 'tags', 'updated_at', 'id', 'user_id', 'content_type_id'];
    }

    // Add filters to data query
    $conditions = [];
    if (isset($filters[0])) {
      $conditions[] = ['title', $filters[0], 'LIKE'];
    }

    // Limit query to content types that user is allowed to view
    if (current_user_can('view_others_h5p_contents') == FALSE) {
      array_push($conditions, ['user_id', get_current_user_id(), '=']);
    }
    ...
    if ($facets !== NULL) {
      ...
      foreach ($facets as $field => $value) {
        if (isset($facetmap[$fields[$field]])) {
          $conditions[] = [$facetmap[$fields[$field]], $value, '='];
        }
      }
    }
  ...
  private function format_tags($tags) {
    ...
    $result  = [];
    $csvtags = ($tags === '' or $tags === NULL) ? [] : explode(';', $tags);
  ...
  public function add_editor_assets($id = NULL) {
    ...
    // Add editor styles
    $styles = is_array(H5peditor::$styles) ? H5peditor::$styles : [];
    foreach ($styles as $style) {
      $assets['css'][] = $url . $style . $cache_buster;
    }

    // Add editor JavaScript
    $scripts = is_array(H5peditor::$scripts) ? H5peditor::$scripts : [];
  ...
  public function ajax_libraries() {
    ...
    // Get input
    $name          = isset($_GET['machineName']) ? sanitize_text_field(wp_unslash($_GET['machineName'])) : NULL;
  ...
  public function ajax_translations() {
    $language = isset($_GET['language']) ? sanitize_text_field(wp_unslash($_GET['language'])) : NULL;
